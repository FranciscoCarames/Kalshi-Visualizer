...
export async function postScan(force: boolean): Promise<{ ok: boolean; code: number; error?: string }> {
  ...
  const r = await fetch(`/scan${force ? "?force=true" : ""}`, { method: "POST", headers: { Accept: "application/json" } });
  if (r.status === 202) return { ok: true, code: 202 };
  let error = `scan ${r.status}`;
  if (r.status === 401) error = "scan locked — token required";
...
export async function getScanStatus(): Promise<ScanStatus> {
  const r = await fetch("/scan/status", { headers: { Accept: "application/json" } });
