...
export type Change = "new" | "up" | "down" | "returned";

export function edgeMap(opps: FeedRow[]): Map<string, number> {
  return new Map(opps.map((o) => [o.id, typeof o.edge === "number" ? o.edge : NaN]));
}

/** Diff vs the previous snapshot. A row absent from the previous snapshot is "returned" if it was seen
 * earlier this session (`seenEver`), else "new". */
export function diffSnapshot(prevEdge: Map<string, number>, opps: FeedRow[], first: boolean, seenEver?: Set<string>):
  ...
  for (const o of opps) {
    ...
    if (!prevEdge.has(o.id)) {
      change.set(o.id, seenEver && seenEver.has(o.id) ? "returned" : "new");
      flash.add(o.id); continue;
    }
