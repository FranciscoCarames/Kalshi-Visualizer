...
export function TerminalProvider({ children }: { children: ReactNode }) {
  ...
  // hideNetNegExec defaults ON (audit Wave 1b): hide executable rows whose TAKER net-of-fees estimate is
  // negative — a display-only declutter that never re-buckets and never hides a MAKER-positive row
  // (net_negative is taker-basis, set only on complete-fee actionable rows), with the hidden-count chip to
  // reveal them. Persisted per-user (auth_store.sanitize_prefs), so this default only affects new/unsaved
  // users — existing saved prefs are preserved (migrate-vs-accept → accept).
  const [settings, setSettings] = useState<Settings>({ longShort: false, showIds: false, resolutionCriteria: true, hideNetNegExec: true, textSize: "normal", tz: "local", autoRefresh: "10s" });
