...
export const COLS: Record<string, Col[]> = {
  opp: [NAME, C("sport", "Sport", "text"), C("detail", "Detail", "text", true), C("action", "Action plan", "text", true),
    ...
    C("trust", "Trust", "text", false, "UNCALIBRATED display heuristic (never an edge / rank / recommendation): a glanceable quality (edge·ROI·ripeness) + confidence (top quote size·quote health·coarse rule basis) grade. Full breakdown in the Inspector; '—' = basis incomplete."),
  ...
  risk: [C("signal", "Signal", "text"), C("flags", "Flags", "text"), C("resolution", "Kind", "text"),
    ...
    C("trust", "Trust", "text", false, "UNCALIBRATED display heuristic (never an edge / rank): quality (edge·ROI·ripeness) + confidence (top quote size·quote health·coarse rule basis) grade. Breakdown in the Inspector; '—' = basis incomplete."),
